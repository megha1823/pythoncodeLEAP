...
#defining the write csv for writing details into it
def WriteInCSVFile(phoneList):
    with open('Residents.csv','w',newline='') as csv_file:
...
def UpdateDetail():
	if E_name.get() and E_last_name.get() and E_contact.get():
		...
		if len(select.curselection())==0:
			...
			message1 = """Press load button to load\n
                          all the information of selected rows\n.
						  """
